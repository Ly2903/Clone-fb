...
const Header = () => {
  ...
  return (
    <header className="bg-nav-bar-bg h-14 px-4 flex justify-between items-center border-color-border border-b-2 fixed w-full z-50">
      <div className="flex items-center w-[300px]">
        <a href="https://www.facebook.com/" className="p-1 inline-block">
          <svg
            aria-label="Logo Facebook"
            className="x1ujvgzy fill-white"
            role="img"
            viewBox="0 0 120 24"
            width="120"
          >
      ...
      <div className="bg-comment-bg rounded-[50px] flex items-center w-[590px] hover:bg-hover-bg">
        <FontAwesomeIcon
          className="pl-3 text-[#B0B3B8] text-[16px]"
          icon={faMagnifyingGlass}
        />
        <input
          className="pt-[7px] pr-2 pb-[9px] pl-2 text-primary-text inline-block flex-1 "
          type="text"
          placeholder="Tìm kiếm trên Facebook"
        />
      </div>
      <div className="flex items-center justify-end w-[360px]">
        <HoverIcon
          className="w-10 h-10 mr-2 bg-secondary-button-bg hover:!bg-hover-bg"
          data-tip="Messenger"
        ...
        >
          <FontAwesomeIcon
            icon={faAngleDown}
            className="bg-[#3a3b3c] text-primary-text text-[12px] p-[2px] rounded-full absolute bottom-0 right-0"
          />
        </HoverImg>
        {showInforSetting && (
          <InforSetting
            showInforSetting={showInforSetting}
            setShowInforSetting={setShowInforSetting}
